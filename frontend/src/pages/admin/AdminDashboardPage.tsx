  ...
  return (
    ...
    >
        ...
        {isLoading ? (
          ...
          <>
            ...
            <StatCard
              ...
              value={s?.totalCourses?.toLocaleString('pt-BR') ?? '—'}
              href="/admin/cursos"
            />
            <StatCard
              label="Matrículas"
              value={s?.totalEnrollments?.toLocaleString('pt-BR') ?? '—'}
              href="/admin/turmas"
            />
            <StatCard
              label="Webhook events"
              value={s?.totalWebhookEvents?.toLocaleString('pt-BR') ?? '—'}
              href="/admin/stripe"
            />
            <StatCard
              label="Webhooks com falha"
              value={s?.failedWebhooks?.toLocaleString('pt-BR') ?? '—'}
              sub={s?.failedWebhooks > 0 ? 'Atenção: reprocessar eventos' : 'Tudo certo'}
              href="/admin/stripe"
            />
            <StatCard
              label="Ações de auditoria"
              value={s?.totalAuditActions?.toLocaleString('pt-BR') ?? '—'}
              href="/admin/stripe"
            />
          </>
        )}
      ...
      <motion.div variants={staggerItem} className="grid sm:grid-cols-2 gap-3">
        <Link
          to="/admin/cursos"
          className="flex items-center gap-3 rounded-[var(--radius-md)] px-4 py-3 bg-[var(--bg-surface-1)] border border-[var(--border-hairline)] hover:border-[var(--border-default)] transition-colors group"
        >
