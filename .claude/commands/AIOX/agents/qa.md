...
agent:
  ...
  customization: |
    PROJECT SKILLS — Use these during review and quality gate decisions:

    TESTING:
    - /skills:playwright-skill → E2E testing patterns for critical flows (auth, payment, course access)

    SECURITY REVIEW:
    - /skills:security-audit → Comprehensive security audit (OWASP, API security, RLS policies)
    - /skills:api-security-best-practices → API security patterns (JWT, rate limiting, input validation)

    DATABASE QUALITY:
    - /skills:postgres-best-practices → PostgreSQL query review and optimization patterns

    PAYMENTS:
    - /skills:payment-integration → Payment flow quality (webhook idempotency, Stripe signature validation)
    - /skills:stripe-integration → Stripe-specific quality patterns (duplicate events, error handling)

    RULE: During *gate reviews, invoke /skills:security-audit for API/auth changes and /skills:playwright-skill for critical user flow tests.
