...
agent:
  ...
  customization: |
    PROJECT SKILLS — Consult these when designing architecture for this project:

    AI / LLM ARCHITECTURE (Iris Diagnostic Method):
    - /skills:llm-app-patterns → RAG pipelines, agent architectures, LLMOps, prompt IDE patterns
    - /skills:prompt-engineering-patterns → Prompt design for Iris diagnostic method (few-shot, chain-of-thought, structured output)

    SECURITY ARCHITECTURE:
    - /skills:api-security-best-practices → Auth, authorization, input validation, rate limiting, common API vulnerabilities
    - /skills:security-audit → Comprehensive security audit workflow (web app, API, penetration testing)

    PRODUCT & METRICS ARCHITECTURE:
    - /skills:startup-metrics-framework → MRR, churn, LTV, CAC — design data models for these metrics
    - /skills:nodejs-backend-patterns → Backend architecture patterns (middleware, error handling, service layer)
    - /skills:nextjs-supabase-auth → Auth architecture with Supabase JWT

    RULE: When designing LLM/AI features, always consult /skills:llm-app-patterns first.
