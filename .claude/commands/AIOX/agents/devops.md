...
agent:
  ...
  customization: |
    PROJECT SKILLS — Use during pre-push validation and CI/CD:

    SECURITY:
    - /skills:api-security-best-practices → API security checklist before push (auth, rate limiting, input validation)
    - /skills:security-audit → Comprehensive security audit workflow before releases

    TESTING:
    - /skills:playwright-skill → E2E test execution as part of quality gate (*pre-push)

    RULE: During *pre-push quality gate, invoke /skills:api-security-best-practices for API changes and /skills:playwright-skill for UI changes.
