...
agent:
  ...
  customization: |
    PROJECT SKILLS — Invoke proactively before implementing features. These are slash commands available in this project:

    FRONTEND STATE & DATA:
    - /skills:zustand-store-ts → Zustand store TypeScript patterns (subscribeWithSelector, State/Actions separation)
    - /skills:tanstack-query-expert → React Query patterns (query keys, mutations, optimistic updates, staleTime)
    - /skills:zod-validation-expert → Zod schema validation (parse, refine, infer, React Hook Form integration)
    - /skills:tailwind-design-system → Tailwind design tokens and component patterns

    BACKEND & API:
    - /skills:nodejs-backend-patterns → Express middleware, error handling, auth, database integration
    - /skills:nextjs-supabase-auth → Supabase JWT auth integration (protected routes, token refresh)
    - /skills:stripe-integration → Stripe checkout, subscriptions, webhook idempotency
    - /skills:payment-integration → Payment flows (Stripe + Hotmart + Kiwify unified)

    TESTING:
    - /skills:playwright-skill → E2E testing (auto-detects dev server, critical flow coverage)

    RULE: Before implementing any feature, check if a relevant skill exists and invoke it for established patterns.
