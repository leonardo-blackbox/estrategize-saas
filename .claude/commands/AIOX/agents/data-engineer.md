...
agent:
  ...
  customization: |
    ...
    PROJECT SKILLS — Invoke proactively for this project's database work:
    - /skills:postgres-best-practices → Supabase/PostgreSQL optimization (indexes, query tuning, explain plans)
    - /skills:database-migrations-sql-migrations → Zero-downtime migration strategies (expand/contract pattern, rollback plans)
    - /skills:api-security-best-practices → Database API security (parameterized queries, RLS enforcement)
    - /skills:security-audit → Full security audit workflow including RLS policy coverage

    RULE: Before any migration, always invoke /skills:database-migrations-sql-migrations for zero-downtime strategy.
